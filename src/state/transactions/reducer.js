...
export default createReducer(initialState, (builder) =>
  builder
    .addCase(addTransaction, (state, { payload: { hash, from, chainId, summary, approval, deposit, claim, deploy, generate } }) => {
      if (state[chainId]?.[hash]) {
        throw new Error('Attempted to add existing transaction.')
      }
      const txs = state[chainId] ?? {}
      txs[hash] = { hash, from, summary, approval, deposit, claim, deploy, generate, addedTime: now() }
      state[chainId] = txs
    })
